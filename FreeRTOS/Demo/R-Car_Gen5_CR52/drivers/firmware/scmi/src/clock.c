...
SCMI_PROTOCOL_DEFINE_NODEV(SCMI_PROTOCOL_CLOCK, NULL);
...
int scmi_clock_rate_set(struct scmi_clock_rate_config *cfg)
{
	...
	msg.hdr = SCMI_MESSAGE_HDR_MAKE(SCMI_CLK_MSG_CLOCK_RATE_SET,
									SCMI_COMMAND, proto->id, 0x0);
	msg.len = sizeof(*cfg);
	msg.content = cfg;
...
int scmi_clock_parent_get(uint32_t clk_id, uint32_t *parent_id)
{
	...
	msg.hdr = SCMI_MESSAGE_HDR_MAKE(SCMI_CLK_MSG_CLOCK_PARENT_GET,
									SCMI_COMMAND, proto->id, 0x0);
	msg.len = sizeof(clk_id);
	msg.content = &clk_id;
...
{
	struct scmi_protocol *proto = &SCMI_PROTOCOL_NAME(SCMI_PROTOCOL_CLOCK);
	struct scmi_clock_parent_config cfg = {.clk_id = clk_id,
										   .parent_id = parent_id};
	struct scmi_message msg, reply;
	int status, ret;
	...
	msg.hdr = SCMI_MESSAGE_HDR_MAKE(SCMI_CLK_MSG_CLOCK_PARENT_SET,
									SCMI_COMMAND, proto->id, 0x0);
	msg.len = sizeof(cfg);
	msg.content = &cfg;
